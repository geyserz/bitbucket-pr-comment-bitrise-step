...
res="$(jq -n -r '{content: { raw: env.bitbucket_pr_comment } }' | curl --write-out %{response_code} -v --output /dev/null --user "${bitbucket_user}:${bitbucket_password}" -X POST -H "Content-Type: application/json" -d @- https://${bitbucket_url}/2.0/repositories/$bitbucket_owner/$bitbucket_repo_slug/pullrequests/$bitbucket_pr_id/comments)"
